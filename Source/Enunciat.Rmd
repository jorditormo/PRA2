...
Veiem com les úniques variables amb anomàlies són "cp" (Tipus de dolor toràcic) i "oldpeak" (Depressió de ST induïda per l'exercici en relació amb el descans). Les dues variables contenen 0 quan no és tracta d'un valor possible per aquestes columnes. Transformem tots aquests 0 a NA:

```{r}
data[data$cp == 0, "cp"] <- NA
data[data$oldpeak == 0, "oldpeak"] <- NA

summary(data$cp)
summary(data$oldpeak)
```

Com les columnes cp i oldpeak tenen més d'un 30% de valors NA, no tindrem en compte aquestes dades per la nostra anàlisi, degut a que no aportarien prou informació rellevant.

```{r}
data <- data[, setdiff(names(data), "cp")]
data <- data[, setdiff(names(data), "oldpeak")]
str(data)

```

Finalment, ens quedem amb 13 variables i 204 registres.

-   **3.2. Identifica i gestiona els valors extrems.**

```{r echo=TRUE, message=FALSE, warning=FALSE}
par(mfrow=c(2,6),mai=c(0.1, 0.0, 0.2, 0.3))
...
```{r}
res_fbs <- boxplot(data$fbs,plot=FALSE)$out
res_fbs
```

```{r}
boxplot(data$thalachh,plot=FALSE)$out
```

```{r}
boxplot(data$caa,plot=FALSE)$out
```

```{r}
boxplot(data$thall,plot=FALSE)$out
```

```{r}
data_clean <- subset(data, !data$trtbps %in% res_trtbps)
data_clean <- subset(data, !data$chol %in% res_chol)
data_clean <- subset(data, !data$fbs %in% res_fbs)
str(data_clean)
```
...
data_num$type <- NULL
data_num_norm <- as.data.frame(lapply(data_num, nor))

summary(data_num_norm)
```
...
res <- optics(data_num_norm, minPts = 12)
res
...
res <- extractDBSCAN(res, eps_cl = 0.29)
res
plot(res)
hullplot(data_num_norm, res)
```
