...
ggplot(data_men, aes(trtbps_n, fill = as.factor(sex))) +
  geom_density(alpha = 0.3) +
   xlim(4.4, 5.3) +
  guides(fill=guide_legend(title="Sex"))+
...
```{r echo=TRUE, message=FALSE, warning=FALSE}
shapiro.test(data_men$thalachh_n)
shapiro.test(data_women$thalachh_n)
```
...
ggplot(df, aes(thalachh_n, fill = as.factor(sex))) +
  geom_density(alpha = 0.3)+
   xlim(0, 20000) +
  guides(fill=guide_legend(title="Sex"))+
...
Es representa gràficament les correlacions entre parells de variables en un conjunt de dades.
...
Primer, factoritzem les variables categòriques i divideix el dataset "desordenat" en 2/3, que s'utilitzaran per l'entrenament, i el terç restant, per testejar.
...
El problema inicialment plantejat és si aquest dataset pot determinar quins són els factors de risc més importants per a la salut cardiovascular.
