...
-   *Edat*: Com la normalitat i l'homoscedasticitat es compleixin (p-valors majors que el nivell de significació), es poden aplicar proves per contrast d'hipòtesis de tipus paramètric, com la prova *t de Student*
  ...
  La hipòtesis nul·la es que les mitjanes d'edat en homes o dones susceptibles a patir un atac de cor o no són iguals:
$$H_0 : \mu_{1h/d} = \mu_{0h/d}$$
...
En ambdós casos, el valor `p` obtingut és menor que el nivell de significança i, per tant, es pot rebutjar la hipòtesi nul·la d’igualtat de mitjanes, i com es suspitava, l'edat és un factor de risc.
...
Es consideren els les mateixes hipòtesis que l'apartat anterior, però en aquest cas en comptes de l'edat es considera lel nivell de colesterol.
...
La hipòtesis nul·la es que les proporcions d'atac al cor entre homes i dones són les mateixes:
