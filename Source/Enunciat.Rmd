...
```{r echo=TRUE, message=FALSE, warning=FALSE}
data_men = data_raw[data_raw$sex == 1,]
data_women = data_raw[data_raw$sex == 0,]
```
...
```{r echo=TRUE, message=FALSE, warning=FALSE}
data_raw["age_segment"] <- cut(data_raw$age, 
                               breaks = c(0,50,120), 
                               labels = c(1, 0))

data_young = data_raw[data_raw$age_segment == 1,]
data_old = data_raw[data_raw$age_segment == 0,]
```
...
El resultat del test permet rebutjar la hipòtesi nul·la d’igualtat de nivells de colesterol i, per tant, es conclou que els nivells de colesterol de les dones són supeiors que la dels homes.
